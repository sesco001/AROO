...
adams({
  ...
  const systemMessage = { role: 'system', content: 'You are an assistant in WhatsApp. You are called PHYNIC💦. You respond to user commands.' };
...
adams({ nomCom: "gpt", reaction: "🦠", categorie: "abu" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;

  try {
    if (!arg || arg.length === 0) {
      return repondre('Hello am Makamesco🖐️.\n\n What help can I offer you today?');
    }
...
adams({ nomCom: "technews", reaction: "📰", categorie: "News" }, async (dest, zk, commandeOptions) => {
  ...
  try {
    ...
    const message = `*VIRUSI MBAYA*\n\n${news}\n\n*Powered by © Virusi Mbaya*`;
...
/*
king({ nomCom: "pea", reaction: "📡", categorie: "User" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
  ...
  try {
    ...
    await repondre('VIRUSI-MDV2 is Getting Your PairingCode.....A Moment!!!');
    const text = encodeURIComponent(arg.join(' '));
    const apiUrl = `/code?number=${text}`;
...
king({ nomCom: "Virusi", reaction: "📡", categorie: "AI" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
          ...
          viewOnceMessage: {
            message: {
              ...
              interactiveMessage: proto.Message.InteractiveMessage.create({
                ...
                footer: proto.Message.InteractiveMessage.Footer.create({
                  text: "> *POWERED BY VIRUSI-MBAYA-MDV2*"
                }),
                header: proto.Message.InteractiveMessage.Header.create({
                  title: "",
                  subtitle: "",
...
adams({
  ...
  try {
    ...
    let caption = '*Powered by MAKAMESCO*';
...
adams({
  ...
  let buttonMessaged = {
    image: { url: url },
    caption: `*POWERED BY VIRUSI-MBAYA-MDV2*`,
  };
...
adams({ nomCom: "nature", reaction: "🦗", categorie: "MAKAMESCO PICS" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
  const response = await fetch('https://api.unsplash.com/photos/random?client_id=72utkjatCBC-PDcx7-Kcvgod7-QOFAm2fXwEeW8b8cc');
const data = await response.json();
  const url =data.urls.regular
...
adams({ nomCom: "enhance", reaction: "💥", categorie: "User" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*Enhanced by MAKAMESCO*';
...
/*king({
  ...
  try {
    ...
    if (result && result.status === 200 && result.result) {
      ...
      let newsText = '*VIRUSI-MBAYA-MDV2 WORLD TECH NEWS*:\n\n';
...
/*adams({ nomCom: "gpt6", reaction: "📡", categorie: "AI" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
...
king({ nomCom: "video", reaction: "🎦", categorie: "AI" }, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;
    ...
    if (response.status === 200 && result && !result.code) {
      ...
      if (extension === 'jpg' || extension === 'jpeg' || extension === 'png') {
        await zk.sendMessage(dest, {
          image: { url: mediaLink },
          caption: "downloaded by VIRUSI MD"
        });
